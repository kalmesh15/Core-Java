class ZomatoExecutor {
	public static void main (String [] args ){

		String foodName;
		double price;

		foodName = "Burger";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Pizza";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Pasta";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Sandwich";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "French Fries";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Hot Dog";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Taco";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Wrap";
		price = Zomato.search(foodName,4);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Noodles";
		price = Zomato.search(foodName,5);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Momos";
		price = Zomato.search(foodName,2);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Paneer Tikka";
		price = Zomato.search(foodName,2);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken Tikka";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Veg Biryani";
		price = Zomato.search(foodName,4);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken Biryani";
		price = Zomato.search(foodName,5);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Mutton Biryani";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Fried Rice";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Manchurian";
		price = Zomato.search(foodName,6);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Spring Roll";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chowmein";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Shawarma";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Falafel";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Grilled Chicken";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "BBQ Wings";
		price = Zomato.search(foodName,5);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Cheese Balls";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Garlic Bread";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Stuffed Paratha";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Dal Tadka";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Paneer Butter Masala";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Butter Chicken";
		price = Zomato.search(foodName,3);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Fish Fry";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Prawns Masala";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Kebab Platter";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Seekh Kebab";
		price = Zomato.search(foodName,3);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken Curry";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Mutton Curry";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Veg Thali";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "NonVeg Thali";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Samosa";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Kachori";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Pav Bhaji";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Dhokla";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Pani Puri";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Bhel Puri";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Rasgulla";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Gulab Jamun";
		price = Zomato.search(foodName,3);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Ice Cream";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Brownie";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chocolate Cake";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Milkshake";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Cold Coffee";
		price = Zomato.search(foodName,2);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Lemon Juice";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Mango Shake";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Fruit Salad";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Omelette";
		price = Zomato.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Boiled Egg";
		price = Zomato.search(foodName,12);
		System.out.println("The food " + foodName + " price is " + price);

	}
}