class SwiggyExecutor {
	public static void main (String [] args ){

		String foodName;
		double price;

		foodName = "Masala Dosa";
		price = Swiggy.search(foodName,5);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Plain Dosa";
		price = Swiggy.search(foodName,3);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Rava Dosa";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Set Dosa";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Onion Dosa";
		price = Swiggy.search(foodName,2);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Idli Sambar";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Vada Sambar";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Rava Idli";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Pongal";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Upma";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Tomato Rice";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Lemon Rice";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Curd Rice";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Bisibele Bath";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Puliyogare";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Akki Roti";
		price = Swiggy.search(foodName,3);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Ragi Mudde";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Veg Pulao";
		price = Swiggy.search(foodName,5);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Jeera Rice";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Ghee Rice";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Kadai Paneer";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Palak Paneer";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Malai Kofta";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Mushroom Masala";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Veg Kolhapuri";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken Lollipop";
		price = Swiggy.search(foodName,6);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken 65";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chilli Chicken";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Dragon Chicken";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Pepper Chicken";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Egg Fried Rice";
		price = Swiggy.search(foodName,6);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken Fried Rice";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Schezwan Rice";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Veg Hakka Noodles";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken Hakka Noodles";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Paneer Roll";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken Roll";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Egg Roll";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Veg Burger";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken Burger";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Club Sandwich";
		price = Swiggy.search(foodName,6);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Grill Sandwich";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Cheese Sandwich";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Veg Momos";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken Momos";
		price = Swiggy.search(foodName,3);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Tandoori Momos";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Paneer Momos";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Veg Soup";
		price = Swiggy.search(foodName,5);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Chicken Soup";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Sweet Corn Soup";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Hot And Sour Soup";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Buttermilk";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Lassi";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Rose Milk";
		price = Swiggy.search(foodName);
		System.out.println("The food " + foodName + " price is " + price);

		foodName = "Badam Milk";
		price = Swiggy.search(foodName,4);
		System.out.println("The food " + foodName + " price is " + price);

	}
}