class NewspaperExecutor {

    public static void main(String[] args) {
        Article a1 = new Article(101,"Tech Growth","Ravi");
        Newspaper n1 = new Newspaper(1,"Times of India",a1);
        
        /*n1.newspaperId = 1;
        n1.newspaperName = "Times of India";
        a1.articleId = 101;
        a1.title = "Tech Growth";
        a1.author = "Ravi";*/
        n1.article = a1;
        n1.getNewspaperDetail();



        Article a2 = new Article(102,"Economic news","Sagar");
        Newspaper n2 = new Newspaper(2,"The Hindu",a2);
		n2.article = a2;
        n2.getNewspaperDetail();
       
       /* 
        n2.newspaperId = 2;
        n2.newspaperName = "The Hindu";
        a2.articleId = 102;
        a2.title = "Economy News";
        a2.author = "Sagar";*/
        

        Article a3 = new Article(103, "Sports update","Rahul");
        Newspaper n3 = new Newspaper(3,"Indian Express",a3);
        
       /* n3.newspaperId = 3;
        n3.newspaperName = "Indian Express";
        a3.articleId = 103;
        a3.title = "Sports Update";
        a3.author = "Rahul";*/
        n3.article = a3;
        n3.getNewspaperDetail();

        Article a4 = new Article(104,"City news","Anil");
		Newspaper n4 = new Newspaper(4,"Deccan Herald",a4);
		n4.article = a4;
        n4.getNewspaperDetail();
        
       /* 
        n4.newspaperId = 4;
        n4.newspaperName = "Deccan Herald";
        a4.articleId = 104;
        a4.title = "City News";
        a4.author = "Anil"; */
        

        Article a5 = new Article(105,"Education","Kiran");
        Newspaper n5 = new Newspaper(5,"Vijay Karnataka",a5);
        
        /*n5.newspaperId = 5;
        n5.newspaperName = "Vijaya Karnataka";
        a5.articleId = 105;
        a5.title = "Education";
        a5.author = "Kiran";
		*/
        n5.article = a5;
        n5.getNewspaperDetail();


        /*Newspaper n6 = new Newspaper();
        Article a6 = new Article();
        n6.newspaperId = 6;
        n6.newspaperName = "Economic Times";
        a6.articleId = 106;
        a6.title = "Market Update";
        a6.author = "Ajay";
        n6.article = a6;
        n6.getNewspaperDetail();


        Newspaper n7 = new Newspaper();
        Article a7 = new Article();
        n7.newspaperId = 7;
        n7.newspaperName = "Business Standard";
        a7.articleId = 107;
        a7.title = "Startup News";
        a7.author = "Neha";
        n7.article = a7;
        n7.getNewspaperDetail();


        Newspaper n8 = new Newspaper();
        Article a8 = new Article();
        n8.newspaperId = 8;
        n8.newspaperName = "The Telegraph";
        a8.articleId = 108;
        a8.title = "Political News";
        a8.author = "Meena";
        n8.article = a8;
        n8.getNewspaperDetail();


        Newspaper n9 = new Newspaper();
        Article a9 = new Article();
        n9.newspaperId = 9;
        n9.newspaperName = "Hindustan Times";
        a9.articleId = 109;
        a9.title = "Health Tips";
        a9.author = "Ramesh";
        n9.article = a9;
        n9.getNewspaperDetail();


        Newspaper n10 = new Newspaper();
        Article a10 = new Article();
        n10.newspaperId = 10;
        n10.newspaperName = "Daily News";
        a10.articleId = 110;
        a10.title = "Technology";
        a10.author = "Varun";
        n10.article = a10;
        n10.getNewspaperDetail();


        Newspaper n11 = new Newspaper();
        Article a11 = new Article();
        n11.newspaperId = 11;
        n11.newspaperName = "Morning Star";
        a11.articleId = 111;
        a11.title = "Environment";
        a11.author = "Priya";
        n11.article = a11;
        n11.getNewspaperDetail();


        Newspaper n12 = new Newspaper();
        Article a12 = new Article();
        n12.newspaperId = 12;
        n12.newspaperName = "Daily Chronicle";
        a12.articleId = 112;
        a12.title = "Science";
        a12.author = "Deepak";
        n12.article = a12;
        n12.getNewspaperDetail();


        Newspaper n13 = new Newspaper();
        Article a13 = new Article();
        n13.newspaperId = 13;
        n13.newspaperName = "Metro Times";
        a13.articleId = 113;
        a13.title = "Transport";
        a13.author = "Nisha";
        n13.article = a13;
        n13.getNewspaperDetail();


        Newspaper n14 = new Newspaper();
        Article a14 = new Article();
        n14.newspaperId = 14;
        n14.newspaperName = "City Express";
        a14.articleId = 114;
        a14.title = "Local News";
        a14.author = "Arjun";
        n14.article = a14;
        n14.getNewspaperDetail();


        Newspaper n15 = new Newspaper();
        Article a15 = new Article();
        n15.newspaperId = 15;
        n15.newspaperName = "Global Times";
        a15.articleId = 115;
        a15.title = "World News";
        a15.author = "Harish";
        n15.article = a15;
        n15.getNewspaperDetail();


        Newspaper n16 = new Newspaper();
        Article a16 = new Article();
        n16.newspaperId = 16;
        n16.newspaperName = "Finance Daily";
        a16.articleId = 116;
        a16.title = "Stock Market";
        a16.author = "Rohit";
        n16.article = a16;
        n16.getNewspaperDetail();


        Newspaper n17 = new Newspaper();
        Article a17 = new Article();
        n17.newspaperId = 17;
        n17.newspaperName = "Weekend Post";
        a17.articleId = 117;
        a17.title = "Entertainment";
        a17.author = "Sneha";
        n17.article = a17;
        n17.getNewspaperDetail();


        Newspaper n18 = new Newspaper();
        Article a18 = new Article();
        n18.newspaperId = 18;
        n18.newspaperName = "Daily Herald";
        a18.articleId = 118;
        a18.title = "Technology Trends";
        a18.author = "Vikram";
        n18.article = a18;
        n18.getNewspaperDetail();


        Newspaper n19 = new Newspaper();
        Article a19 = new Article();
        n19.newspaperId = 19;
        n19.newspaperName = "City Chronicle";
        a19.articleId = 119;
        a19.title = "Public Issues";
        a19.author = "Ajith";
        n19.article = a19;
        n19.getNewspaperDetail();


        Newspaper n20 = new Newspaper();
        Article a20 = new Article();
        n20.newspaperId = 20;
        n20.newspaperName = "Daily World";
        a20.articleId = 120;
        a20.title = "Global Economy";
        a20.author = "Ananya";
        n20.article = a20;
        n20.getNewspaperDetail();*/

    }
}